...
void MultiNodeSparseGraphGroup<Builder>::initFirstRun(Ptr<data::Batch> batch, bool launchServerThread) {
  MultiNodeGraphGroup::initFirstRun(batch, false);
...
void MultiNodeSparseGraphGroup<Builder>::initRemoteCommunicationVars(bool initBuffers) { // @TODO: Integrate with clients / drop-rate / comm-overlap
  ...
  for (int gpu = 0; gpu < devices_.size(); gpu++) {
    size_t size = nodeShardSizes_[mpi_my_rank_] * 3 * (1.0 - min(0.99, dropRate_));
